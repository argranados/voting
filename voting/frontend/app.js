...
function renderNominees(nominees, roundId){
  ...
  nominees.forEach((n)=>{
    ...
    col.innerHTML = `<div class="card nominee-card border"><div class="card-body">
      ...
      <div class="fs-4 fw-bold mb-3">${n.contestantId}</div>
      <button class="btn btn-outline-success" data-round-id="${roundId}" data-contestant-id="${n.contestantId}">Votar por ${n.contestantId}</button>
    </div></div>`;
...
function renderResults(resultDto){
  ...
  resultDto.results.forEach((item, index)=>{
    ...
    row.innerHTML=`<div><div class="small-label">Posición ${index+1}</div><div class="fw-semibold">Contestant ${item.contestantId}</div></div><span class="badge text-bg-primary rounded-pill">${item.votes} votos</span>`;
